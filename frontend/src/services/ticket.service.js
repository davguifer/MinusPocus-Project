...
import "../static/css/player/ticketPage.css";
...
class TicketService {
    getTicketList([tickets, setTickets], auth, [alerts, setAlerts], setMessage, setVisible, plan = null) {
        return tickets.map((t, index) => {
            ...
            const removePlayerVet = () => deleteFromList(`/api/v1/consultations/${t.consultation.id}/tickets/${t.id}`, t.id, [tickets, setTickets],
            ...
            let buttons;
            if (auth === "PLAYER") {
                buttons = index === length - 1 && plan === "PLATINUM" && t.user.authority.authority === "PLAYER" && status !== "CLOSED" ?
                    <ButtonGroup>
                        <Button aria-label={"edit-" + t.id} size="sm" color="primary" tag={Link}
                            to={`consultations/${t.consultation.id}/tickets/${t.id}`}>
                            Edit
                        </Button>
                        <Button aria-label={"delete-" + t.id} size="sm" color="danger" onClick={removePlayerVet}>
                            Delete
                        </Button>
                    </ButtonGroup> :
                    <></>;
            ...
            const alignment = t.user?.authority?.authority === "PLAYER" ?
                "me-auto" :
                "ms-auto";
            const style = t.user?.authority?.authority === "PLAYER" ?
